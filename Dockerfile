...
RUN apk -U add gcc musl-dev python3-dev libffi-dev openssl-dev cargo py3-pip curl apk-cron tzdata \ 
  ...
  && echo '        domain-connect-dyndns update --domain all --config $CONFFILE  ' >> /usr/local/bin/chkip    \
  && echo '        curl -s ifconfig.me > $IPFILE' >> /usr/local/bin/chkip    \
  ...
  && echo "        echo 'Config file $CONFFILE does not exist you can launch in a terminal the folowing command : domainSetup.sh '"  >> /usr/local/bin/entrypoint.sh  \ 
  && echo "        echo 'and the restart the container. Or you can copy past following commands from container log'"  >> /usr/local/bin/entrypoint.sh  \ 
